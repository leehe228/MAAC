...
class AttentionSAC(object):
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        for pi, ob in zip(self.target_policies, next_obs): # for all agent, each agents' target policy & next observation
            curr_next_ac, curr_next_log_pi = pi(ob, return_log_pi=True) # agent�� target policy network (next obs) -> ������Ʈ�� policy log_pi, action ��ȯ
        ...
        for a_i, nq, log_pi, (pq, regs) in zip(range(self.nagents), next_qs, next_log_pis, critic_rets):
            target_q = (rews[a_i].view(-1, 1) + # rews[a_i] shape�� (?, 1)�� reshape
                        self.gamma * nq *
    ...
    def update_all_targets(self):
        ...
        """
        soft_update(self.target_critic, self.critic, self.tau) # critic network�� �̿��� target critic network�� ������Ʈ (central critic network)
        for a in self.agents:
            soft_update(a.target_policy, a.policy, self.tau) # �� agent�� ���� policy network�� �̿��� �� agent�� target policy network�� ������Ʈ
